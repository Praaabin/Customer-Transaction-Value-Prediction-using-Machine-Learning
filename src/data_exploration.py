...
def explore_data(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col, title in dist_cols:
        ...
        sns.histplot(subset[col], kde=True, bins=30, color='skyblue')
        plt.title(f"Distribution of {title}", fontsize=14)
        plt.xlabel(title, fontsize=12)
        plt.ylabel("Frequency", fontsize=12)
        plt.axvline(subset[col].mean(), color='red', linestyle='--', label='Mean')
        plt.legend()
    ...
    sns.barplot(data=top_countries, x="Transactions", y="Country", palette="viridis")
    ...
    sns.lineplot(data=monthly, x="Month", y="Transactions", marker="o", color='green')
